...
export async function fetchUsuarios(): Promise<Usuario[]> {
  try {
    return await sql<Usuario[]>`SELECT * FROM tbladmins WHERE active IS NULL`;
  } catch (error) {
...
export async function fetchUsuarioById(id: string): Promise<Usuario | null> {
  try {
    const result = await sql<Usuario[]>`SELECT * FROM tbladmins WHERE id = ${id} AND active IS NULL`;
